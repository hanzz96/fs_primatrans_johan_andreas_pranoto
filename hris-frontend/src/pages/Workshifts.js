...
function WorkShifts() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (form.id) {
                await dispatch(
                    updateWorkshift({ id: form.id, data: form })
                ).unwrap();
            } else {
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Are you sure to delete this workshift?")) {
            try {
                await dispatch(deleteWorkshift(id)).unwrap();
                dispatch(
                    fetchWorkshifts({
                        page: paginationModel.page + 1,
                        perPage: paginationModel.pageSize,
        ...
        <div>
            ...
            <div style={{ height: 500, width: "100%" }}>
                <DataGrid
                    ...
                    pageSizeOptions={[5, 10, 15, 20, 100]} // updated options
